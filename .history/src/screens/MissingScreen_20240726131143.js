...
const MissingScreen = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      people = people.map(person => ({
        ...person,
        createdAt: new Date(person.createdAt.seconds * 1000) // Convert Firestore timestamp to Date object
      }));
      people.sort((a, b) => b.createdAt - a.createdAt); // Sort by createdAt descending
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity style={styles.card} onPress={() => toggleComments(item.id)}>
      ...
      <Text style={styles.createdAt}>Posted: {item.createdAt.toLocaleDateString()} {item.createdAt.toLocaleTimeString()}</Text>
