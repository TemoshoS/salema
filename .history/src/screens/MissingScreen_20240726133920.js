...
const MissingScreen = () => {
  ...
  const heightAnims = useRef({}).current;
  ...
  const renderItem = ({ item }) => {
    ...
    if (!heightAnims[item.id]) {
      heightAnims[item.id] = new Animated.Value(0);
    }

    useEffect(() => {
      Animated.timing(heightAnims[item.id], {
    ...
    return (
      ...
      >
        ...
        <Animated.View style={{ height: heightAnims[item.id], overflow: 'hidden' }}>
