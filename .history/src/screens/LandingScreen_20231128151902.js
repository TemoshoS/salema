...
const LandingScreen = ({ navigation, visible }) => {
  ...
  const validatePhoneNumber = (phoneNumber) => {
    // Ensure the phone number starts with "27" and remove leading "0"
    const formattedPhoneNumber = phoneNumber.startsWith("0")
      ? "27" + phoneNumber.slice(1)
      : phoneNumber;
  
    // You can use a regular expression to validate the phone number
    const phoneRegex = /^27[0-9]{9}$/; // Adjust the regex for your specific format
