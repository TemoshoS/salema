...
import { getPeople, addPerson, getComments, addCommentToPerson } from '../services/missingService';
...
const MissingScreen = () => {
  ...
  const handleComment = async (person) => {
    setSelectedPerson(person);
    const fetchedComments = await getComments(person.id);
  ...
  const handleAddComment = async () => {
    try {
      ...
      const newComment = { user: 'User', comment }; // You can replace 'User' with the current user's name or ID.
      await addCommentToPerson(selectedPerson.id, newComment);

      setComment('');
      const updatedComments = await getComments(selectedPerson.id);
  ...
  const handleAddPerson = async () => {
    try {
      ...
      if (!newPerson.names || !newPerson.lastSeen || !newPerson.age) {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.comment}>
              <Text style={styles.commentText}><Text style={styles.commentUser}>{item.user}:</Text> {item.comment}</Text>
            </View>
          )}
