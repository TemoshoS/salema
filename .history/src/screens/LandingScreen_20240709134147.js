...
const LandingScreen = ({ navigation, visible }) => {
  ...
  const handleRemoveContact = (contactId) => {
    setContactIdToRemove(contactId); // Set the contactId to remove
    setIsConfirmationVisible(true); // Show the confirmation modal
  };
  
  // Updated confirmRemoveContact function
  const confirmRemoveContact = async (confirmed) => {
    setIsConfirmationVisible(false); // Hide the confirmation modal
    if (confirmed) {
      try {
        // Call removeContact with contactIdToRemove
        await removeContact(contactIdToRemove).then(() => {
          fetchContacts(); // Fetch updated contacts after removal
          bottomSheetModalRef.current?.close(); // Close the bottom sheet if open
          Toast.show({
            type: 'success',
        ...
        Toast.show({
          type: 'error',
          text1: 'Emergency contact not removed',
  ...
  onPress={() => handleRemoveContact(contact.id)} // Pass contact.id to handleRemoveContact
>
    ...
    <View style={styles.modalContent}>
      ...
      <View style={styles.modalButtons}>
        <TouchableOpacity
          ...
          onPress={() => confirmRemoveContact(true)} // Confirm deletion
        >
          <Text style={styles.buttonText}>YES</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.modalButton}
          onPress={() => setIsConfirmationVisible(false)} // Cancel deletion
        >
