...
export class ShakeEventExpo {
  static async addListener(handler) {
    ...
    if (!currentUser) {
      console.log('User is not logged in. Shake detection disabled.');
      return;
    }
...
const sendShakeDetectedSms = async (location) => {
  try {
    ...
    const message = `Hi it's currentUser ,I'm in danger I need help. My location: https://www.google.com/maps?q=${location.coords.latitude},${location.coords.longitude}`;
