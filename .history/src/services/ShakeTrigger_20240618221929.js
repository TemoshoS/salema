        ...
        if (speed > THRESHOLD) {
          ...
          if (currTime - lastNotificationTime > 5 * 60 * 1000) { // 5 minutes in milliseconds
            ...
            Location.getCurrentPositionAsync({})
              .then(async location => {
                try {
                  // Replace with your Twilio number and message content
                  const from = '+12086266619'; // Your Twilio number
                  const message = 'Emergency: Device shaken, location: ' + location.coords.latitude + ', ' + location.coords.longitude;
                  
                  // Add your recipient numbers here or use existing state/props
                  const numbers = ['+27721371977', '+27724457811']; // Example numbers
