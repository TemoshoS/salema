...
const sendShakeDetectedSms = async (location) => {
  try {
    const currentUser = await getCurrentUser(); // Adjust this function to fetch the current user
    if (!currentUser) {
      console.error('No current user found.');
      return;
    }

    const message = `Hi, it's ${currentUser.displayName}. Emergency! I need help. My location: https://www.google.com/maps?q=${location.coords.latitude},${location.coords.longitude}`;
