...
const sendShakeDetectedSms = async (location) => {
  try {
    ...
    if (!currentUser || !currentUser.name) {
      console.error('No current user or name found.');
      return;
    }

    // Prepare message with user's name and location
    const message = `Hi, it's ${currentUser.name}. I'm in d555555anger and need help. My location: https://www.google.com/maps?q=${location.coords.latitude},${location.coords.longitude}`;
    const from = '+12086266619'; // Replace with your Twilio number or sender ID

    // Fetch phone numbers for current user
    ...
    // Wait for all SMS requests to complete
